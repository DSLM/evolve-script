...
// @match        https://g8hh.github.io/evolve/''',
    ...
    'addSettingsToggle(currentNode, "prestigeMADIgnoreArpa", "Ignore early game A.R.P.A.", "Disables building any A.R.P.A. projects until MAD is researched, or rival have appeared");':'addSettingsToggle(currentNode, "prestigeMADIgnoreArpa", "特定时期之前不建造ARPA项目", "研究相互毁灭或竞争国家出现之前，不建造ARPA项目");',
    ...
    'addSettingsSelect(currentNode, "prioritizeOuterFleet", "Ship Yard Blueprint (The True Path)", "Alter script behaviour to assist fleet building, prioritizing resources required for current design of ship.", priority);':'addSettingsSelect(currentNode, "prioritizeOuterFleet", "外域船坞(智械黎明模式)", "调整脚本分配舰队建筑的方式，优先舰船缺失的资源。", priority);',
    ...
    # NOTE: 生产设置
    'addSettingsNumber(currentNode, "productionChrysotileWeight", "Chrysotile weighting", "Chrysotile weighting for autoQuarry, applies after adjusting to difference between current amounts of Stone and Chrysotile");':'addSettingsNumber(currentNode, "productionChrysotileWeight", "温石棉权重", "自动温石棉控制使用的权重，根据当前的石头和温石棉差值来应用权重");',
    'addStandardHeading(currentNode, "Smelter");':'addStandardHeading(currentNode, "冶炼厂");',
    '[{val: "iron", label: "Prioritize Iron", hint: "Produce only Iron, untill storage capped, and switch to Steel after that"},':'[{val: "iron", label: "优先熔炼铁", hint: "只冶炼铁，直到铁达到存储上限，再切换为冶炼钢"},',
    '{val: "steel", label: "Prioritize Steel", hint: "Produce as much Steel as possible, untill storage capped, and switch to Iron after that"},':'{val: "steel", label: "优先熔炼钢", hint: "只冶炼钢，直到钢达到存储上限，再切换为冶炼铁"},',
    '{val: "storage", label: "Up to full storages", hint: "Produce both Iron and Steel at ratio which will fill both storages at same time for both"},':'{val: "storage", label: "直到达到上限", hint: "以一定的比例同时冶炼铁和钢，保证它们同时达到存储上限"},',
    '{val: "required", label: "Up to required amounts", hint: "Produce both Iron and Steel at ratio which will produce maximum amount of resources required for buildings at same time for both"}];':'{val: "required", label: "直到达到需求数量", hint: "以一定的比例同时冶炼铁和钢，保证它们同时达到建筑的需求"}];',
    'addSettingsSelect(currentNode, "productionSmelting", "Smelters production", "Distribution of smelters between iron and steel", smelterOptions);':'addSettingsSelect(currentNode, "productionSmelting", "冶炼厂生产", "冶炼厂冶炼铁和钢的方式", smelterOptions);',
    'addSettingsNumber(currentNode, "productionSmeltingIridium", "Iridium ratio (The True Path)", "Share of smelters dedicated to Iridium");':'addSettingsNumber(currentNode, "productionSmeltingIridium", "铱冶炼比例(智械黎明模式)", "用于冶炼铱的比例");',
    '>Fuel<':'>燃料使用顺序<',
    'productionElement.append(buildTableLabel(fuel.id));':'productionElement.append(buildTableLabel({"Oil":"石油","Coal":"煤","Wood":"木材","Star":"星辰","Inferno":"地狱燃料"}[fuel.id]));',
    'addStandardHeading(currentNode, "Foundry");':'addStandardHeading(currentNode, "铸造厂");',
    '[{val: "none", label: "None", hint: "Use configured weightings with no additional adjustments, craftables with x2 weighting will be crafted two times more intense than with x1, etc."},':'[{val: "none", label: "None", hint: "按照正常的权重制造。2倍权重的锻造物将比1倍权重的锻造物多制造1倍，以此类推。"},',
    '{val: "demanded", label: "Prioritize demanded", hint: "Ignore craftables once stored amount surpass cost of most expensive building, until all missing resources will be crafted. After that works as with \'none\' adjustments."},':'{val: "demanded", label: "优先制造需要的", hint: "当锻造物储量超过花费最高的建筑时忽略相应锻造物，直到所有锻造物都超过了相应数值。之后与上方“无”选项效果相同。"},',
    '{val: "buildings", label: "Buildings weightings", hint: "Uses weightings of buildings which are waiting for craftables, as multipliers to craftables weighting. This option requires autoBuild."}];':'{val: "buildings", label: "按建筑权重", hint: "使用需要锻造物建筑的权重，计入锻造物的权重。需要开启自动建筑此项才能生效。"}];',
    'addSettingsSelect(currentNode, "productionFoundryWeighting", "Weightings adjustments", "Configures how exactly craftables will be weighted against each other", weightingOptions);':'addSettingsSelect(currentNode, "productionFoundryWeighting", "锻造物权重", "控制锻造物与其他资源相比的权重", weightingOptions);',
    '>Min Materials<':'>Min Materials<',
    'addStandardHeading(currentNode, "Factory");':'addStandardHeading(currentNode, "工厂");',
    'addSettingsNumber(currentNode, "productionFactoryMinIngredients", "Minimum materials to preserve", "Factory will craft resources only when all required materials above given ratio");':'addSettingsNumber(currentNode, "productionFactoryMinIngredients", "原料保底产量", "工厂只在所有需要的材料都高于保底产量时制造相应产品");',
    'addStandardHeading(currentNode, "Mining Drone");':'addStandardHeading(currentNode, "采矿机器人");',
    '<span>Managed</span>':'<span>脚本自动管理</span>',

    # NOTE: 工作设置
    'addSettingsToggle(currentNode, "jobSetDefault", "Set default job", "Automatically sets the default job in order of Quarry Worker -> Lumberjack -> Crystal Miner -> Scavenger -> Hunter -> Farmer");':'addSettingsToggle(currentNode, "jobSetDefault", "设置默认工作", "自动以石工->伐木工人->水晶矿工->清道夫->猎人->农民的顺序设置默认工作");',
    'addSettingsNumber(currentNode, "jobLumberWeighting", "Final Lumberjack Weighting", "AFTER allocating breakpoints this weighting will be used to split lumberjacks, quarry workers, crystal miners and scavengers");':'addSettingsNumber(currentNode, "jobLumberWeighting", "最终伐木工人权重", "用于分配伐木工人，石工，水晶矿工和清道夫的数量");',
    'addSettingsNumber(currentNode, "jobQuarryWeighting", "Final Quarry Worker Weighting", "AFTER allocating breakpoints this weighting will be used to split lumberjacks, quarry workers, crystal miners and scavengers");':'addSettingsNumber(currentNode, "jobQuarryWeighting", "最终石工权重", "用于分配伐木工人，石工，水晶矿工和清道夫的数量");',
    'addSettingsNumber(currentNode, "jobCrystalWeighting", "Final Crystal Miner Weighting", "AFTER allocating breakpoints this weighting will be used to split lumberjacks, quarry workers, crystal miners and scavengers");':'addSettingsNumber(currentNode, "jobCrystalWeighting", "最终水晶矿工权重", "用于分配伐木工人，石工，水晶矿工和清道夫的数量");',
    'addSettingsNumber(currentNode, "jobScavengerWeighting", "Final Scavenger Weighting", "AFTER allocating breakpoints this weighting will be used to split lumberjacks, quarry workers, crystal miners and scavengers");':'addSettingsNumber(currentNode, "jobScavengerWeighting", "最终清道夫权重", "用于分配伐木工人，石工，水晶矿工和清道夫的数量");',
    'addSettingsToggle(currentNode, "jobDisableMiners", "Disable miners in Andromeda", "Disable Miners and Coal Miners after reaching Andromeda");':'addSettingsToggle(currentNode, "jobDisableMiners", "到达仙女座星系以后禁用矿工", "到达仙女座星系以后禁用矿工和煤矿工人");',
    'addSettingsToggle(currentNode, "jobDisableCraftsmans", "Craft manually when possible", "Disable foundry crafters when manual craft is allowed");':'addSettingsToggle(currentNode, "jobDisableCraftsmans", "如果可以的话，手动进行锻造", "如果可以手动进行锻造，则禁用可手动锻造资源的所有工匠");',
    '>Job<':'>工作<',
    '>1st Pass Max<':'>第一阈值<',
    '>2nd Pass Max<':'>第二阈值<',
    '>Final Max<':'>最终阈值<',
    'Unemployed: new Job("unemployed", "Unemployed"),':'Unemployed: new Job("unemployed", "失业人口"),',
    'Hunter: new Job("hunter", "Hunter"),':'Hunter: new Job("hunter", "猎人"),',
    'Farmer: new Job("farmer", "Farmer"),':'Farmer: new Job("farmer", "农民"),',
    'Lumberjack: new Job("lumberjack", "Lumberjack"),':'Lumberjack: new Job("lumberjack", "伐木工人"),',
    'QuarryWorker: new Job("quarry_worker", "Quarry Worker"),':'QuarryWorker: new Job("quarry_worker", "石工"),',
    'CrystalMiner: new Job("crystal_miner", "Crystal Miner"),':'CrystalMiner: new Job("crystal_miner", "水晶矿工"),',
    'Scavenger: new Job("scavenger", "Scavenger"),':'Scavenger: new Job("scavenger", "清道夫"),',
    'Colonist: new Job("colonist", "Colonist"),':'Colonist: new Job("colonist", "行星居民"),',
    'TitanColonist: new Job("titan_colonist", "Titan Colonist"),':'TitanColonist: new Job("titan_colonist", "卫星行星居民"),',
    'Miner: new Job("miner", "Miner"),':'Miner: new Job("miner", "矿工"),',
    'CoalMiner: new Job("coal_miner", "Coal Miner"),':'CoalMiner: new Job("coal_miner", "煤矿工人"),',
    'CementWorker: new Job("cement_worker", "Cement Worker"),':'CementWorker: new Job("cement_worker", "水泥工人"),',
    'Professor: new Job("professor", "Professor"),':'Professor: new Job("professor", "教授"),',
    'Scientist: new Job("scientist", "Scientist"),':'Scientist: new Job("scientist", "科学家"),',
    'Entertainer: new Job("entertainer", "Entertainer"),':'Entertainer: new Job("entertainer", "艺人"),',
    'HellSurveyor: new Job("hell_surveyor", "Hell Surveyor"),':'HellSurveyor: new Job("hell_surveyor", "勘探者"),',
    'SpaceMiner: new Job("space_miner", "Space Miner"),':'SpaceMiner: new Job("space_miner", "太空矿工"),',
    'Archaeologist: new Job("archaeologist", "Archaeologist"),':'Archaeologist: new Job("archaeologist", "考古学家"),',
    'Banker: new Job("banker", "Banker"),':'Banker: new Job("banker", "银行家"),',
    'Priest: new Job("priest", "Priest"),':'Priest: new Job("priest", "牧师"),',
    'Plywood: new CraftingJob("Plywood", "Plywood Crafter", resources.Plywood),':'Plywood: new CraftingJob("Plywood", "胶合板工匠", resources.Plywood),',
    'Brick: new CraftingJob("Brick", "Brick Crafter", resources.Brick),':'Brick: new CraftingJob("Brick", "砌砖工匠", resources.Brick),',
    'WroughtIron: new CraftingJob("Wrought_Iron", "Wrought Iron Crafter", resources.Wrought_Iron),':'WroughtIron: new CraftingJob("Wrought_Iron", "锻铁工匠", resources.Wrought_Iron),',
    'SheetMetal: new CraftingJob("Sheet_Metal", "Sheet Metal Crafter", resources.Sheet_Metal),':'SheetMetal: new CraftingJob("Sheet_Metal", "金属板工匠", resources.Sheet_Metal),',
    'Mythril: new CraftingJob("Mythril", "Mythril Crafter", resources.Mythril),':'Mythril: new CraftingJob("Mythril", "秘银工匠", resources.Mythril),',
    'Aerogel: new CraftingJob("Aerogel", "Aerogel Crafter", resources.Aerogel),':'Aerogel: new CraftingJob("Aerogel", "气凝胶工匠", resources.Aerogel),',
    'Nanoweave: new CraftingJob("Nanoweave", "Nanoweave Crafter", resources.Nanoweave),':'Nanoweave: new CraftingJob("Nanoweave", "纳米织物工匠", resources.Nanoweave),',
    'Scarletite: new CraftingJob("Scarletite", "Scarletite Crafter", resources.Scarletite),':'Scarletite: new CraftingJob("Scarletite", "绯绯色金工匠", resources.Scarletite),',
    'Quantium: new CraftingJob("Quantium", "Quantium Crafter", resources.Quantium),':'Quantium: new CraftingJob("Quantium", "量子工匠", resources.Quantium),',

    # NOTE: 建筑设置
    'addSettingsToggle(currentNode, "buildingsIgnoreZeroRate", "Do not wait for resources without income", "Weighting checks will ignore resources without positive income(craftables, inactive factory goods, etc), buildings with such resources will not delay other buildings.");':'addSettingsToggle(currentNode, "buildingsIgnoreZeroRate", "忽略无产量的资源", "权重将忽略无产量的资源(例如锻造物，未进行生产的产物等)，如果有相应的建筑物需要这些资源，则不会因此影响其他建筑的建造。");',
    'addSettingsToggle(currentNode, "buildingsLimitPowered", "Limit amount of powered buildings", "With this option enabled Max Build will prevent powering extra building. Can be useful to disable buildings with overrided settings.");':'addSettingsToggle(currentNode, "buildingsLimitPowered", "限制需要供能的建筑数量", "开启此项后，脚本只会对建造上限数量的建筑进行供能，超出部分不进行供能。可以用来限制以其他方式建造的建筑供能上限。");',
    'addSettingsNumber(currentNode, "buildingTowerSuppression", "Minimum suppression for Towers", "East Tower and West Tower won\'t be built until minimum suppression is reached");':'addSettingsNumber(currentNode, "buildingTowerSuppression", "巨塔安全指数阈值", "达到相应安全指数以后，才会开始建造西侧巨塔和东侧巨塔");',
    '[{val: "any", label: "Any", hint: "Build any Shrines, whenever have resources for it"},':'[{val: "any", label: "任意类型", hint: "只要资源足够就建造圣地"},',
    '{val: "equally", label: "Equally", hint: "Build all Shrines equally"},':'{val: "equally", label: "平均分配", hint: "平均建造所有类型的圣地"},',
    '{val: "morale", label: "Morale", hint: "Build only Morale Shrines"},':'{val: "morale", label: "士气", hint: "只建造提升士气的圣地"},',
    '{val: "metal", label: "Metal", hint: "Build only Metal Shrines"},':'{val: "metal", label: "金属", hint: "只建造提升金属产量的圣地"},',
    '{val: "know", label: "Knowledge", hint: "Build only Knowledge Shrines"},':'{val: "know", label: "知识", hint: "只建造提升知识的圣地"},',
    '{val: "tax", label: "Tax", hint: "Build only Tax Shrines"}];':'{val: "tax", label: "税收", hint: "只建造提升税收的圣地"}];',
    'addSettingsSelect(currentNode, "buildingShrineType", "Magnificent Shrine", "Auto Build shrines only at moons of chosen shrine", shrineOptions);':'addSettingsSelect(currentNode, "buildingShrineType", "圣地种类偏好", "只在对应月相时建造相应的圣地", shrineOptions);',
    '>Building<':'>建筑物<',
    ...
    # NOTE: 自动建筑权重设置
    'addSettingsToggle(currentNode, "buildingBuildIfStorageFull", "Ignore weighting and build if any storage is full", "Ignore weighting and immediately construct building if it uses any capped resource, preventing wasting them by overflowing. Weight still need to be positive(above zero) for this to happen.");':'addSettingsToggle(currentNode, "buildingBuildIfStorageFull", "如果任意相关资源存储已满，则忽略权重进行建造", "如果建筑所使用的任意一项资源超过上限，则忽略权重立刻进行建造，以避免浪费资源。权重仍然需要设为正数(大于0)后此项才能生效。");',
    '>Target<':'>目标<',
    '>Condition<':'>条件<',
    '>Multiplier<':'>倍率<',
    'addWeightingRule(tableBodyNode, "Any", "New building", "buildingWeightingNew");':'addWeightingRule(tableBodyNode, "任意类型", "新建筑", "buildingWeightingNew");',
    'addWeightingRule(tableBodyNode, "Powered building", "Low available energy", "buildingWeightingUnderpowered");':'addWeightingRule(tableBodyNode, "用电建筑", "电力不足", "buildingWeightingUnderpowered");',
    'addWeightingRule(tableBodyNode, "Power plant", "Low available energy", "buildingWeightingNeedfulPowerPlant");':'addWeightingRule(tableBodyNode, "发电厂", "电力不足", "buildingWeightingNeedfulPowerPlant");',
    'addWeightingRule(tableBodyNode, "Power plant", "Producing more energy than required", "buildingWeightingUselessPowerPlant");':'addWeightingRule(tableBodyNode, "发电厂", "电力过剩", "buildingWeightingUselessPowerPlant");',
    'addWeightingRule(tableBodyNode, "Knowledge storage", "Have unlocked unafforable researches", "buildingWeightingNeedfulKnowledge");':'addWeightingRule(tableBodyNode, "知识上限建筑", "存在因知识上限不足而无法进行的研究", "buildingWeightingNeedfulKnowledge");',
    'addWeightingRule(tableBodyNode, "Knowledge storage", "All unlocked researches already affordable", "buildingWeightingUselessKnowledge");':'addWeightingRule(tableBodyNode, "知识上限建筑", "不存在因知识上限不足而无法进行的研究", "buildingWeightingUselessKnowledge");',
    'addWeightingRule(tableBodyNode, "Building with state (city)", "Some instances of this building are not working", "buildingWeightingNonOperatingCity");':'addWeightingRule(tableBodyNode, "需要调整供能的建筑(地面)", "并非所有建筑都在正常供能", "buildingWeightingNonOperatingCity");',
    'addWeightingRule(tableBodyNode, "Building with state (space)", "Some instances of this building are not working", "buildingWeightingNonOperating");':'addWeightingRule(tableBodyNode, "需要调整供能的建筑(太空)", "并非所有建筑都在正常供能", "buildingWeightingNonOperating");',
    'addWeightingRule(tableBodyNode, "Building with consumption", "Missing consumables to operate", "buildingWeightingMissingSupply");':'addWeightingRule(tableBodyNode, "供能物资不足的建筑", "缺少供能物资，无法正常运转", "buildingWeightingMissingSupply");',
    'addWeightingRule(tableBodyNode, "Support consumer", "Missing support to operate", "buildingWeightingMissingSupport");':'addWeightingRule(tableBodyNode, "需要花费支持的建筑", "缺少支持，无法正常运转", "buildingWeightingMissingSupport");',
    'addWeightingRule(tableBodyNode, "Support provider", "Provided support not currently needed", "buildingWeightingUselessSupport");':'addWeightingRule(tableBodyNode, "提供支持的建筑", "提供的支持超过了目前的需求", "buildingWeightingUselessSupport");',
    'addWeightingRule(tableBodyNode, "All fuel depots", "Missing Oil or Helium for techs and missions", "buildingWeightingMissingFuel");':'addWeightingRule(tableBodyNode, "所有燃料存储", "进行研究或任务需要的石油或氦-3超过存储上限", "buildingWeightingMissingFuel");',
    'addWeightingRule(tableBodyNode, "Not housing, barrack, oil derrick, or knowledge building", "MAD prestige enabled, and affordable", "buildingWeightingMADUseless");':'addWeightingRule(tableBodyNode, "提升人口、士兵、石油或知识上限以外的建筑", "进行核爆重置，且已研究相互毁灭", "buildingWeightingMADUseless");',
    'addWeightingRule(tableBodyNode, "Mass Ejector", "Existed ejectors not fully utilized", "buildingWeightingUnusedEjectors");':'addWeightingRule(tableBodyNode, "质量喷射器", "存在未完全运作的质量喷射器", "buildingWeightingUnusedEjectors");',
    'addWeightingRule(tableBodyNode, "Freight Yard, Container Port, Munitions Depot", "Have unused crates or containers", "buildingWeightingCrateUseless");':'addWeightingRule(tableBodyNode, "货场、集装箱港口与弹药库", "有未使用的板条箱或集装箱", "buildingWeightingCrateUseless");',
    'addWeightingRule(tableBodyNode, "Horseshoes", "No more Horseshoes needed", "buildingWeightingHorseshoeUseless");':'addWeightingRule(tableBodyNode, "马蹄铁", "暂时不需要马蹄铁", "buildingWeightingHorseshoeUseless");',
    'addWeightingRule(tableBodyNode, "Meditation Chamber", "No more Meditation Space needed", "buildingWeightingZenUseless");':'addWeightingRule(tableBodyNode, "冥想室", "暂时不需要冥想室", "buildingWeightingZenUseless");',
    'addWeightingRule(tableBodyNode, "Gate Turret", "Gate demons fully supressed", "buildingWeightingGateTurret");':'addWeightingRule(tableBodyNode, "远古之门炮塔", "远古之门的恶魔已经完全压制", "buildingWeightingGateTurret");',
    'addWeightingRule(tableBodyNode, "Warehouses, Garage, Cargo Yard, Storehouse", "Need more storage", "buildingWeightingNeedStorage");':'addWeightingRule(tableBodyNode, "仓库，格纳库，星际货仓，卫星仓库", "需要更多提供储量上限的建筑", "buildingWeightingNeedStorage");',
    'addWeightingRule(tableBodyNode, "Housing", "Less than 90% of houses are used", "buildingWeightingUselessHousing");':'addWeightingRule(tableBodyNode, "住房", "有市民居住的住房没有超过90%", "buildingWeightingUselessHousing");',

    # NOTE: ARPA设置
    'addSettingsToggle(currentNode, "arpaScaleWeighting", "Scale weighting with progress", "Projects weighting scales  with current progress, making script more eager to spend resources on finishing nearly constructed projects.");':'addSettingsToggle(currentNode, "arpaScaleWeighting", "进度权重", "随着项目接近完成而提高权重，使脚本更优先进行接近完成的项目。");',
    'addSettingsNumber(currentNode, "arpaStep", "Preferred progress step", "Projects will be weighted and build in this steps. Increasing number can speed up constructing. Step will be adjusted down when preferred step above remaining amount, or surpass storage caps. Weightings below will be multiplied by current step. Projects builded by triggers will always have maximum possible step.");':'addSettingsNumber(currentNode, "arpaStep", "每次建造进度百分比", "每次建造时建造相应百分比的项目。触发器永远使用100%的百分比。");',
    '>Project<':'>项目<',
    ...
    'StarPower: new StarPower("Star Power", "StarPower")':'StarPower: new StarPower("星辰", "StarPower")',
